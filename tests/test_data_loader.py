...
# ---------------------------------------------------------------------------
# Fixture: parque mínimo con SRM en pasillo + STV en ambos anillos
# ---------------------------------------------------------------------------
...
def tablas():
    equipos = pd.DataFrame([
        ...
        {"id": "STV-E-01", "tipo": "STV", "zona": "anillo_entrada", "estado_operativo": "operativo"},
        {"id": "STV-S-01", "tipo": "STV", "zona": "anillo_salida",  "estado_operativo": "operativo"},
    ])
    ...
    misiones = pd.DataFrame([
        ...
        {"id_mision": 2, "id_equipo": "SRM-02",   "posicion_inicial": "A", "posicion_final": "B",
         ...
         "estado": "completada"},
        {"id_mision": 3, "id_equipo": "STV-E-01", "posicion_inicial": "A", "posicion_final": "B",
         "ts_inicio": pd.Timestamp("2025-06-10 09:00"), "ts_fin": pd.Timestamp("2025-06-10 09:02"),
         "estado": "completada"},
        {"id_mision": 4, "id_equipo": "STV-S-01", "posicion_inicial": "A", "posicion_final": "B",
    ...
    eventos = pd.DataFrame([
        ...
        {"id_evento": 3, "id_equipo": "STV-E-01", "codigo_error": "E01",
         "ts_inicio_fallo": pd.Timestamp("2025-06-10 09:00"),
         "ts_recuperacion": pd.Timestamp("2025-06-10 09:30"), "estado": "resuelto"},
        {"id_evento": 4, "id_equipo": "STV-S-01", "codigo_error": "E01",
...
class TestFiltroTipo:
    ...
    def test_propaga_a_misiones_y_eventos(self, tablas):
        """Filtrar por SRM debe descartar misiones/eventos de STV."""
        out = aplicar_filtros_globales(tablas, tipos_equipo=["SRM"])
        assert set(out["misiones"]["id_equipo"]) == {"SRM-01", "SRM-02"}
        assert set(out["eventos"]["id_equipo"]) == {"SRM-01", "SRM-02"}
...
class TestFiltroZona:
    def test_solo_anillo_salida(self, tablas):
        out = aplicar_filtros_globales(tablas, zonas=["anillo_salida"])
        assert set(out["equipos"]["id"]) == {"STV-S-01"}
        assert set(out["misiones"]["id_equipo"]) == {"STV-S-01"}
        assert set(out["eventos"]["id_equipo"]) == {"STV-S-01"}

    def test_combina_tipo_y_zona(self, tablas):
        """tipos_equipo=[STV] + zonas=[anillo_entrada] → solo STV-E-01."""
        out = aplicar_filtros_globales(
            tablas, tipos_equipo=["STV"], zonas=["anillo_entrada"]
        )
        assert set(out["equipos"]["id"]) == {"STV-E-01"}
